...
public abstract class GamMojoModelBase extends MojoModel implements ConverterFactoryProvidingModel {
  ...
  int _lastClass;
  int _featuresPresentInDataCount = 0; // Count number of predictors found both in input data and Gam Algo predictor
  
  public int getTotFeatureSize() { return _totFeatureSize;}
  public String[] getNames() { return _names; }  // get column names centered
  public int getFeautesPresentInData() { return _featuresPresentInDataCount;}
  
  GamMojoModelBase(String[] columns, String[][] domains, String responseColumn) {
    super(columns, domains, responseColumn);
  }

  public void countFeautesPresentInData(Set<String> inputColumnNames) {
     for (String cName : features()) {
      if (inputColumnNames.contains(cName))
        _featuresPresentInDataCount++; // count column names that are present both in input data row and gam predictor names
    }
  }
